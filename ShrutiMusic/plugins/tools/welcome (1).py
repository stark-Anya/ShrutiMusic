...
def welcomepic(pic, user, chat, id, uname):
    # Local path se background load karo
    ...
    cx = int(bg_w * 0.755)        # ← left/right adjust karo
    cy = int(bg_h * 0.470)        # ← upar/neeche adjust karo
    pfp_x = cx - pfp_size // 2
    pfp_y = cy - pfp_size // 2
    ...
    text_start_y = int(bg_h * 0.390)        # ← text ka starting Y
    line_gap     = 72                        # ← lines ke beech gap

    uname_str  = f"@{uname}" if uname else "Not Set"
    name_clean = unidecode(user)[:20]
